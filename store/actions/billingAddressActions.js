...
export const SAVE_BILL_ADDRESS = "SAVE_BILL_ADDRESS";
...
export const initBillingAddressList = () => {
  ...
  if (user) {
    AddressList = JSON.parse(localStorage.getItem("BillingshoppingAddressList"))
...
export const saveBillAddress = (Address) => {
  ...
  console.log(
    "bbbbbbbbb Address at billingAddressActions SAVE_ADDRESS address action 11111111111111111111:",
    newList
  );
  localStorage.setItem("BillingshoppingAddressList", JSON.stringify(newList));
  return { type: SAVE_BILL_ADDRESS, AddressList: newList };
};

export const saveBillingGoogleAddress = (Address) => {
  // const AddressList = JSON.parse(localStorage.getItem('shoppingAddressList'))
  //   ? JSON.parse(localStorage.getItem('shoppingAddressList'))
  //   : [];
  const newList = [Address];
  // console.log(' Address at SAVE_ADDRESS address action :', newList);
  localStorage.setItem("BillingsaveAddress", JSON.stringify(newList));
...
export const removeAddress = (Address) => {
  ...
  var tempAddress = JSON.parse(
    localStorage.getItem("BillingshoppingAddressList")
  );
  const length = tempAddress.length;
  if (length === 1) {
    localStorage.removeItem("BillingshoppingAddressList");
    tempAddress = [];
  } else {
