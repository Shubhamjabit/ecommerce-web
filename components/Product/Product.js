...
const Product = ({ product, urlPath }) => {
  ...
  if (type == "cableLength") {
    ...
    if (!isNaN(value) && value >= cableLengthLimit.min && value <= cableLengthLimit.max) {
      setCableLength(parseFloat(value.toFixed(2)));
    }
  } else {
    const value = parseInt(evnt.target.value);
    if (!isNaN(value) && value >= qtyLimit.min && value <= qtyLimit.max) {
      setQty(value);
    }
  }
};
